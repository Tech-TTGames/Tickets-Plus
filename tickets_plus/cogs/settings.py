...
class Settings(commands.GroupCog,
    ...
    async def change_tracked(self, ctx: discord.Interaction,
        ...
        async with self._bt.get_connection() as conn:
            new, ticket_user = await conn.get_ticket_bot(user.id, ctx.guild.id
                                                        )  # type: ignore
            if not new:
    ...
    async def change_staff(self, ctx: discord.Interaction, role: discord.Role):
        ...
        """
        async with self._bt.get_connection() as conn:
            new, staff_role = await conn.get_staff_role(role.id, ctx.guild.id
                                                       )  # type: ignore
            if not new:
    ...
    async def change_observers(self, ctx: discord.Interaction,
        ...
        """
        async with self._bt.get_connection() as conn:
            new, obsrvrs = await conn.get_observers_role(role.id, ctx.guild.id
                                                        )  # type: ignore
            if not new:
    ...
    async def change_community_roles(self, ctx: discord.Interaction,
        ...
        """
        async with self._bt.get_connection() as conn:
            new, comsup = await conn.get_community_role(role.id, ctx.guild.id
                                                       )  # type: ignore
            if not new:
    ...
    async def change_community_ping_roles(self, ctx: discord.Interaction,
        ...
        """
        async with self._bt.get_connection() as conn:
            new, comsup = await conn.get_community_ping(role.id, ctx.guild.id
                                                       )  # type: ignore
            if not new:
                await conn.delete(comsup)
